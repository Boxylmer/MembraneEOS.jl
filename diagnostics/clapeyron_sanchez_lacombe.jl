...
@show v = volume(co2_pure_model, p, t)  # 2.136649093323808 L/mol
                      # Clapeyron returns 2.1366490933238094 L/mol --> normal a_res, new mixing rule
...
@show v = volume(co2_ch4_model, p, t, z)  # 2.216132223322739 L/mol
                        # Clapeyron returns 2.211124407739013 L/mol --> normal a_res, new mixing rule
...
@show v = volume(co2_ch4_model_with_kij, p, t, z)  # 2.205729481036781 L/mol
                                 # Clapeyron returns 2.2013115585553185 L/mol --> normal a_res, new mixing rule
...
@show v = volume(model_pdms, p, t)  #  100.37221812334239 L/mol
                   # Clapeyron returns 100.37221812334239 L/mol --> normal a_res, new mixing rule
...
@show v = volume(model_pdms_co2, p, t, z)  # 18.108297158036713 L/mol
                         # Clapeyron returns 18.08932995502047  L/mol --> normal a_res, new mixing rule
